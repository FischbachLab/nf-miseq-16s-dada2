...
sel_stat_df=stat[['input', 'filtered', 'nonchim']]  #.set_index('sample')
total_corrected=stat[['nonchim']]
# read ASV table
...
#df2['asv_counts/Filtered_reads_counts(%)'] = round(asv.iloc[:, 0:].sum(axis=1)/total_corrected.iloc[:,0]*100,2)
...
sample_dict = {}
sample_list = df2.index[df2['Reads_counts_genus_level/total_corrected_reads(%)'] < 100 ].tolist()
...
# Added the ASV as the 1st row in output table
sum_df.loc[' '] = ["","", "", "", "", "", "", "", "", "", "", "", "",""] + merged_ASVs_list
newIndex=[' ']+[ind for ind in sum_df.index if ind!=' ']
sum_df=sum_df.reindex(index=newIndex)
#Rename col names
#sum_df.columns.values[0] = 'Sample'
#sum_df.rename(columns = {" " : "Sample"}, inplace = True)
sum_df.index.name = 'Sample_ID'
sum_df.rename(columns = {"input" : "Input_reads"}, inplace = True)
sum_df.rename(columns = {"filtered" : "Filtered_reads"}, inplace = True)
sum_df.rename(columns = {"nonchim" : "Corrected_reads"}, inplace = True)
sum_df[['1st_Domain','1st_Phylum','1st_Class','1st_Order','1st_Family','1st_Genus']] = sum_df.iloc[:, 7].str.split(";",expand=True)
